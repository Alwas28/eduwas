...
/* ── Stats ── */
.stats-row{display:flex;gap:24px;padding-top:28px;border-top:1px solid var(--border);margin-top:32px}
.stat-num{font-family:'Clash Display',sans-serif;font-size:22px;font-weight:700;background:linear-gradient(135deg,var(--ac),var(--ac2));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
...
.f-input{
  ...
  border-radius:11px;padding:11px 14px;font-size:13.5px;font-family:inherit;outline:none;
...
.check-label{display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px;color:var(--sub)}
.check-label input{accent-color:var(--ac);width:15px;height:15px}
...
<div class="auth-wrap">
  ...
  <div class="auth-left">
    ...
    <div class="left-content">
      ...
      <div>
        <div class="feature-item">
          ...
          <div>
            ...
            <div class="feature-desc">Akses semua konten perkuliahan kapan saja dan di mana saja</div>
          </div>
        </div>
        <div class="feature-item">
          ...
          <div>
            <div class="feature-title">AI Grading Otomatis</div>
            <div class="feature-desc">Penilaian cerdas dengan feedback personal untuk setiap jawaban</div>
          </div>
        </div>
        <div class="feature-item">
          ...
          <div>
            ...
            <div class="feature-desc">Lacak nilai, kehadiran, dan perkembangan belajarmu</div>
          </div>
        </div>
        <div class="feature-item">
          ...
          <div>
            <div class="feature-title">AI Tutor Personal</div>
            <div class="feature-desc">Tanya jawab seputar materi kapan saja, dijawab oleh AI</div>
          </div>
        </div>
      </div>
      ...
      <div class="stats-row">
        <div>
          <div class="stat-num">{{ \App\Models\Mahasiswa::count() }}+</div>
          <div class="stat-label">Mahasiswa</div>
        </div>
        <div>
          <div class="stat-num">{{ \App\Models\Instruktur::where('status','aktif')->count() }}</div>
          <div class="stat-label">Instruktur</div>
        </div>
        <div>
          <div class="stat-num">{{ \App\Models\MataKuliah::where('aktif',true)->count() }}</div>
  ...
  <div class="auth-right">
    <div class="auth-form-box">
      ...
      <p class="form-sub">Belum punya akun? <a href="{{ route('register') }}" style="color:var(--ac);font-weight:600;text-decoration:none;">Daftar sekarang</a></p>

      @if(session('status'))
      <div style="background:rgba(var(--ac-rgb),.1);border:1px solid rgba(var(--ac-rgb),.25);color:var(--ac);border-radius:10px;padding:10px 14px;font-size:13px;margin-bottom:20px;display:flex;align-items:center;gap:8px;">
      ...
      <form method="POST" action="{{ route('login') }}" id="login-form">
        ...
        <div class="f-group">
          ...
          @error('email')
          <p class="f-error"><i class="fas fa-circle-exclamation" style="font-size:10px;"></i> {{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="f-group">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
            ...
            <a href="{{ route('password.request') }}" style="font-size:12px;color:var(--ac);text-decoration:none;font-weight:600;">Lupa password?</a>
            @endif
          </div>
          <div class="pw-wrap">
      ...
      <p style="text-align:center;font-size:12.5px;color:var(--muted);">
        <a href="/" style="color:var(--muted);text-decoration:none;display:inline-flex;align-items:center;gap:5px;transition:color .15s"
...
<script>
function togglePw() {
  const input = document.getElementById('pw-login');
