...
Route::middleware('auth')->group(function () {
    // PIN-based email verification (replaces signed-URL flow)
    Route::get('verify-pin', [VerifyPinController::class, 'show'])
        ->name('verification.notice');

    Route::get('verify-pin/check', [VerifyPinController::class, 'check'])
        ->name('verification.pin.check');

    Route::post('verify-pin', [VerifyPinController::class, 'verify'])
        ->middleware('throttle:10,1')
        ->name('verification.pin.verify');

    Route::post('verify-pin/resend', [VerifyPinController::class, 'resend'])
